...
def brodie_similarity (x, y, df_bag_of_frames, match, mismatch, gap):
    ...
    if x[i] in df_bag_of_frames.columns :
        ratio_factor_stack1 = sum([(1 - df_bag_of_frames[x[i]].sum() / df_bag_of_frames.shape[0]) * (1 - k / nx) for k in range(nx)])
    else :
        ratio_factor_stack1 = sum([(1 - i / nx) for i in range(nx)])
    
    if y[j] in df_bag_of_frames.columns :   
        ratio_factor_stack2 = sum([(1 - df_bag_of_frames[y[j]].sum() / df_bag_of_frames.shape[0]) * (1 - k / nx) for k in range(ny)])
    else :
        ratio_factor_stack2 = sum([(1 - i / nx) for j in range(ny)])
